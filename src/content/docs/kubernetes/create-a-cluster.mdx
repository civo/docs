...
:::note
You will need to have set the correct [Civo region](/overview/regions/) for where the cluster was created when you [set up Civo CLI](/overview/civo-cli/), or specify it in the command with `--region` to be able to view the cluster information.
:::
...
Once you have configured the [Civo Terraform provider](/overview/terraform/), you can define a Civo Kubernetes resource in Terraform:
