...
In this section, you can select the number of nodes to create in this new pool, and the specifications/size of the nodes to create. You can choose from the same sizes as when [creating a cluster](/kubernetes/create-a-cluster/).
...
Once you have configured the [Civo Terraform provider](/overview/terraform/) and successfully created a [Kubernetes cluster resource](/kubernetes/create-a-cluster/) in Terraform, you can define a new node pool as follows.
...
- *Create a firewall* and *Create a firewall rule*
  - Creates a new firewall and firewall rule for the Kubernetes cluster. Specifically, we are opening port 6443 so we can communicate with our cluster from outside e.g. through the `kubectl` CLI. See the [firewall creation](/networking/firewalls/) documentation for more information.
