...
If you are creating a new Kubernetes cluster, you can add the cluster autoscaler as part of the applications to add using the `-a` parameter. Please refer to the [cluster creation documentation](/kubernetes/create-a-cluster/) for more information.
...
For more information Civo CLI, see the [CLI documentation](/overview/civo-cli/).
</TabItem>
...
Refer to the [cluster creation documentation](/kubernetes/create-a-cluster/) using Terraform for more options and required fields.
...
:::note
Your [account quota](/account/quota/) will take precedence over any node scaling, and you would begin seeing "Quota Limit Reached" errors in the Cloud Controller Manager pod if the autoscaler would try to scale the node count beyond what your quota allows for.
:::
