...
A [cluster running on Civo](/kubernetes/create-a-cluster/) will have `civo-volume` as the default storage class. This can be confirmed by viewing the `storageclass` resources on your cluster:
...
:::caution
If you have any external volumes attached to a [cluster you delete](/kubernetes/delete-a-cluster/), these will remain in your account for use. They are not automatically removed. As such, any external volumes will [be charged](/account/billing/) until they are explicitly deleted.
:::