...
Kubernetes clusters on Civo will remain in your account until you delete them. Clusters in your account are [billed hourly](/account/billing/) for each node and count towards your [account quota](/account/quota/). If you no longer need a Kubernetes cluster, you can delete it in the following ways.
...
:::caution
If you have any external [volumes](kubernetes-volumes) attached to the cluster when it is deleted, these will remain in your account for use. They are not automatically removed. As such, any external volumes will [be charged](/account/billing/) until they are explicitly deleted.
:::
</TabItem>
