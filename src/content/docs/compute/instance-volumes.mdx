...
:::note
Volumes are region and network-specific. If you create a volume in one [private network](/networking/private-networks/), it can only be mounted and used by instances in that network.
:::
...
By default, the volume is created in the `default` network in your currently-selected region. You can use the switch `--network NETWORK_ID` to specify a [private network](/networking/private-networks/) in which to create the volume.
